...
静的バッファの再利用、メモリ割り当てオーバーヘッドゼロのカスタムLuaアロケータ、リアルタイムの積極的なガベージコレクション（GC）など、アグレッシブなベアメタルメモリハックを採用することにより、ESP32-S3の制限された320KB RAMの壁を打ち破ります。これにより、スタンドアロンのシステムBIOSの起動や、様々なPICO-8カートリッジを手のひらの上で動かすことができます。<br>
...
厳しいメモリ制限があるため、**大きな `.p8` ファイルをそのままSDカードに入れても起動しません。** 事前に最適化を行う必要があります。

### 1. 推奨: ShrinkO8 を使ったコードの圧縮 (Minification)
ほとんどのゲームは、ソースコードを短く圧縮（Minify）するだけで、実機上の限られたメモリでもコンパイルして遊べるようになります。

まずは **[ShrinkO8](https://thisismypassport.github.io/shrinko8/)** をメインのツールとして使用することを強くおすすめします。
1. ブラウザで [ShrinkO8](https://thisismypassport.github.io/shrinko8/) を開きます。
2. 遊びたい `.p8` または `.p8.png` ファイルを読み込ませます。
3. **Minify** を実行し、コメントや空白を削除してコードを極限まで圧縮します。
4. 保存された `.p8` ファイルを microSD カードに入れて使用してください。

...
#### 1. リポジトリのクローン（サブモジュールの取得）
本プロジェクトは、必要な依存モジュール（`zepto8` などのサブモジュール）を含んでいるため、クローンする際は `--recursive` フラグを使用してください。
```bash
git clone --recursive <リポジトリのURL>
```
既にサブモジュールを含めずに通常クローンしてしまっている場合は、以下のコマンドを実行してサブモジュールの初期化と更新を行ってください。
```bash
git submodule update --init --recursive
```

#### 2. プロジェクトの読み込み
VSCode を起動し、本プロジェクトのルートディレクトリ（`platformio.ini` が存在するフォルダ）を開きます。PlatformIO が自動的にプロジェクトを認識します。

...
//
//  Cardputer/ESP32 adaptation:
//  - Replaced <lol/math> with "compat/lol_compat.h"
//  - Removed HAVE_CONFIG_H block
//  - lol::rand(lo, hi) implemented locally using stdlib random
//  - All waveform generation math preserved exactly
//
...
float synth::waveform(pico8::state::synth_param &params)
{
    ...
    switch (params.instrument)
    {
        ...
        case INST_SAW:
            ...
            if (buzz)
                ret = ret * 0.83f - (fabsf(fmodf(advance, 2.f) - 1.0f) < 0.5f ? 0.085f : 0.0f);
            return 0.653f * ret;
        ...
        case INST_NOISE:
        {
            ...
            float new_sample = (params.last_sample + scale * lol::rand(-1.f, 1.f))
                               / (1.0f + scale);
        ...
        {   // sum of two triangle waves with a slightly different frequency
            ...
            ret += 1.f - fabsf(4.f * fmodf(advance * 109.f / 110.f, 1.f) - 2.f);
            if (buzz)
            {
                // original triangle has freq 1, 3, 5, 7, 9 ...
                // add waves at 2, 6, 10, 14
                ret += 0.25f - fabsf(1.f * fmodf(advance * 2.0f + 0.5f, 1.f) - 0.5f);
                // add waves at 4, 12, 20, 28
                ret += 0.125f - fabsf(0.5f * fmodf(advance * 4.0f, 1.f) - 0.25f);
            }
            return ret / 6.f;
        }
    }

...
//
//  ESP32/Arduino port: lol/math replaced with compat/lol_compat.h
//  lol::round() → std::round(), lol::mix/lol::clamp provided by lol_compat.h
//

#include "compat/lol_compat.h"
#include <algorithm> // std::swap
#include <cmath>     // std::min, std::max, std::round
                  ...
                  opt<fix32> arg3, opt<fix32> arg4)
{
    ...
    // lol::clamp<int> is provided by lol_compat.h
    ...
    for (;;)
    {
        ...
        if (horiz)
        {
            if (x == xend)
                break;
            x += dx;
            // lol::mix<double> is provided by lol_compat.h; std::round replaces lol::round
            y = (int16_t)std::round(lol::mix((double)y0, (double)y1, (double)(x - x0) / (x1 - x0)));
        }
        else
        {
            if (y == yend)
                break;
            y += dy;
            x = (int16_t)std::round(lol::mix((double)x0, (double)x1, (double)(y - y0) / (y1 - y0)));
        }
    }
}
                 ...
                 opt<int16_t> in_cel_w, opt<int16_t> in_cel_h, int16_t layer)
{
    ...
    int16_t cel_w = no_size ? map_size_x : *in_cel_w;
    int16_t cel_h = no_size ? (m_ram.hw_state.mapping_map >= 0x80 ? map_size_y : map_size_y / 2) : *in_cel_h;

    u4mat2<128, 128>& gfx = m_ram.get_gfx();
    u4mat2<128, 128>& screen = get_current_screen();

    // Fast path is active when no fillp or bit_mask is set
    bool fast_path = (hw.bit_mask == 0) && (!ds.fillp_flag || ((ds.fillp[0] == 0) && (ds.fillp[1] == 0)));

    // Iterate over map tiles instead of pixels to avoid redundant lookups
    for (int16_t ty = 0; ty < cel_h; ++ty)
    {
        int16_t map_y = cel_y + ty;
        if (map_y < 0 || map_y >= map_size_y) continue;

        int16_t start_dy = sy + ty * 8;
        if (start_dy + 8 <= ds.clip.y1 || start_dy >= ds.clip.y2) continue;

        for (int16_t tx = 0; tx < cel_w; ++tx)
        {
            int16_t map_x = cel_x + tx;
            if (map_x < 0 || map_x >= map_size_x) continue;

            int16_t start_dx = sx + tx * 8;
            if (start_dx + 8 <= ds.clip.x1 || start_dx >= ds.clip.x2) continue;

            uint8_t sprite = m_ram.map[map_size_x * map_y + map_x];
            if (sprite == 0 && !ds.misc_features.sprite_zero) continue;

            uint8_t bits = m_ram.gfx_flags[sprite];
            if (layer && !(bits & layer)) continue;

            int16_t spr_src_x = sprite % 16 * 8;
            int16_t spr_src_y = sprite / 16 * 8;

            // Pre-compute per-tile clip ranges
            int16_t lx_start = (start_dx < ds.clip.x1) ? (ds.clip.x1 - start_dx) : 0;
            int16_t lx_end   = (start_dx + 8 > ds.clip.x2) ? (ds.clip.x2 - start_dx) : 8;
            int16_t ly_start = (start_dy < ds.clip.y1) ? (ds.clip.y1 - start_dy) : 0;
            int16_t ly_end   = (start_dy + 8 > ds.clip.y2) ? (ds.clip.y2 - start_dy) : 8;

            for (int16_t ly = ly_start; ly < ly_end; ++ly)
            {
                int16_t screen_y = start_dy + ly;
                const uint8_t* src_row = gfx.data[spr_src_y + ly];
                uint8_t*       dst_row = screen.data[screen_y];

                for (int16_t lx = lx_start; lx < lx_end; ++lx)
                {
                    int16_t src_x = spr_src_x + lx;
                    uint8_t col = (src_row[src_x >> 1] >> ((src_x & 1) ? 4 : 0)) & 0xf;
                    uint8_t pal_entry = ds.draw_palette[col];

                    if ((pal_entry & 0xf0) == 0)
                    {
                        uint8_t color = pal_entry & 0xf;
                        int16_t screen_x = start_dx + lx;
                        if (fast_path)
                        {
                            uint8_t& p = dst_row[screen_x >> 1];
                            p = (p & (screen_x & 1 ? 0x0f : 0xf0)) | (screen_x & 1 ? color << 4 : color);
                        }
                        else
                        {
                            uint32_t color_bits = color << 16;
                            set_pixel(screen_x, screen_y, color_bits);
                        }
                    }
                }
                 ...
                 opt<fix32> h, bool flip_x, bool flip_y)
{
    ...
    // Pre-compute clip bounds for the destination rect
    int16_t x_start = (x < ds.clip.x1) ? (ds.clip.x1 - x) : 0;
    int16_t x_end   = (x + w8 > ds.clip.x2) ? (ds.clip.x2 - x) : w8;
    int16_t y_start = (y < ds.clip.y1) ? (ds.clip.y1 - y) : 0;
    int16_t y_end   = (y + h8 > ds.clip.y2) ? (ds.clip.y2 - y) : h8;
    if (x_start >= x_end || y_start >= y_end) return;

    // Sprite sheet origin
    int16_t base_sx = (n % 16) * 8;
    int16_t base_sy = (n / 16) * 8;

    if (fast_path && !flip_x && !flip_y)
    {
        // ── Fastest path: no flipping ──────────────────────────
        for (int16_t j = y_start; j < y_end; ++j)
        {
            int16_t src_y   = base_sy + j;
            int16_t screen_y = y + j;
            const uint8_t* src_row = gfx.data[src_y];
            uint8_t*       dst_row = screen.data[screen_y];

            for (int16_t i = x_start; i < x_end; ++i)
            {
                int16_t src_x = base_sx + i;
                // Inline nibble read from 4-bit packed buffer
                uint8_t col = (src_row[src_x >> 1] >> ((src_x & 1) ? 4 : 0)) & 0xf;
                uint8_t pal_entry = ds.draw_palette[col];
                if ((pal_entry & 0xf0) == 0) // not transparent
                {
    ...
    {
        // ── General path: flipping or non-fast ─────────────────
        for (int16_t j = y_start; j < y_end; ++j)
        {
            int16_t screen_y = y + j;
            int16_t dj = flip_y ? h8 - 1 - j : j;
            int16_t src_y = base_sy + dj;

            for (int16_t i = x_start; i < x_end; ++i)
            {
                int16_t screen_x = x + i;
                int16_t di = flip_x ? w8 - 1 - i : i;
                int16_t src_x = base_sx + di;

                uint8_t col = gfx.get(src_x, src_y);
                uint8_t pal_entry = ds.draw_palette[col];
                ...
                if ((pal_entry & 0xf0) == 0)
                {
                    ...
                    if (fast_path)
                    {
                        uint8_t& p = screen.data[screen_y][screen_x >> 1];
                        p = (p & (screen_x & 1 ? 0x0f : 0xf0)) | (screen_x & 1 ? color << 4 : color);
                    }
                    else
                    {

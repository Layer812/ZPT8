// pico8/vm.h — PICO-8 virtual machine header
...
// ──────────────────────────────────────────────────────────────
// VM state (everything serialisable outside of Lua/PICO-8 RAM)
// ──────────────────────────────────────────────────────────────
struct state
{
    int   buttons[3][64];
...
public:
    ...
    virtual float getTime() override { return api_time(); }

virtual std::string const &get_code() const;
virtual std::string       &get_mutable_code();
    ...
    bool render_fast(uint16_t *dest) const;
    ...
    {
        m_fullscreen = value;
        if (save) m_configfile.set_dirty();
        if (runCallback && setfullscreen_callback) setfullscreen_callback(value);
    }
...
private:
    ...
    // No network download on Cardputer
    tup<bool, bool, std::string> private_download(opt<std::string> str)
    {
        return {true, false, std::string("no network")};
    }
    struct { int step; std::string cart_path; } download_state = {};
    ...
    void    api_line(opt<fix32> arg0, opt<fix32> arg1, opt<fix32> arg2, opt<fix32> arg3, opt<fix32> arg4);
    ...
    void    api_spr(int16_t n, int16_t x, int16_t y, opt<fix32> w, opt<fix32> h, bool flip_x, bool flip_y);
    void    api_sspr(int16_t sx, int16_t sy, int16_t sw, int16_t sh,
                     int16_t dx, int16_t dy, opt<int16_t> in_dw, opt<int16_t> in_dh,
...
public:
    // Exported API table (used by bindings/lua.h)
...
private:
    ...
    void update_sfx_state(state::sfx_state &cur_sfx, state::synth_param &new_synth,
...
private:
    ...
    breadcrumb_path breadcrumbs_buf[MAX_BREADCRUMBS];
    int breadcrumbs_count = 0;

    double m_time = 0;
    // On ESP32 we use millis() instead of steady_clock
    uint32_t m_timer_last_ms = 0;
    ...
    char m_ui_texts[12][32] = {
        ...
        "filter", "back", "continue", "reset cart", "quit",
        "windowed", "full"
    };

    // Custom stat callbacks - small, static array
    static constexpr int MAX_STATS = 16;
    struct stat_entry { int16_t id; std::function<std::any()> fn; };
    stat_entry m_stats_buf[MAX_STATS];
    int m_stats_count = 0;
    // extcmds not used on Cardputer, remove to save memory

    bool m_watch_file_change = false;
    bool m_exit_requested    = false;
    bool m_is_running        = true;
    bool m_auto_run          = false;
};

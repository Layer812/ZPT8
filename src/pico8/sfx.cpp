...
static std::string key_to_name(float key)
{
    ...
    char buf[16];
    std::snprintf(buf, sizeof(buf), "%s%d", note, octave);
...
void vm::update_sfx_state(state::sfx_state& cur_sfx, state::synth_param &new_synth, float freq_factor, float length, bool is_music, bool can_loop, bool half_rate, float inv_frames_per_second)
{
    ...
    if (loop_range > 0.f && next_offset >= sfx.loop_end
        && can_loop)
    {
        next_offset = fmodf(next_offset - sfx.loop_start, loop_range)
            + sfx.loop_start;
    }
    ...
    if (offset < 32)
    {
        ...
        if (volume > 0.f)
        {
            ...
            case FX_ARP_SLOW:
            {
                ...
                freq = key_to_freq((float)sfx.notes[arp_note].key);
                break;
            }
            }
...
void vm::get_audio(void *inbuffer, size_t in_bytes)
{
    ...
    // Pre-calculate frame-constant values to avoid repetitive operations inside the loop
    ...
    for (size_t i = 0; i < (size_t)in_frames; ++i)
    {
        float channel_mix = 0.0f;
        for (int chan = 0; chan < 4; ++chan)
        {
            float const inv_f = inv_frames_per_second[chan];

            if (chan == 0 && m_state.music.pattern != -1 && !is_pause)
            {
                float const offset_per_frame = offset_per_second_c0 * inv_f;
                m_state.music.offset += offset_per_frame;
                m_state.music.fade_volume += m_state.music.fade_volume_step * inv_f;
                m_state.music.fade_volume = lol::clamp(m_state.music.fade_volume, 0.f, 1.f);

                if (m_state.music.fade_volume_step < 0.f && m_state.music.fade_volume <= 0.f)
                {
                    set_music_pattern(-1);
                }
            ...
            if (channel_state.main_sfx.sfx == -1 && channel_state.sfx_music != -1 && !is_pause)
            {
                ...
                bool want_play = true;
                int const speed = max(1, (int)sfx.speed);
                float new_offset = m_state.music.offset / speed;

                float const loop_range = float(sfx.loop_end - sfx.loop_start);
                if (loop_range > 0.f && channel_state.can_loop)
                {
                    if (new_offset > sfx.loop_start)
                        new_offset = fmodf(new_offset - sfx.loop_start, loop_range) + sfx.loop_start;
                }
                else
                {
                    if (new_offset > 32.f)
                ...
                if (want_play)
                {
                    launch_sfx(index, chan, new_offset, 0.f, true);
                }
            ...
            if (!is_pause)
            {
                ...
                update_sfx_state(channel_state.main_sfx, new_synth, 1.0f, channel_state.length, channel_state.is_music, channel_state.can_loop, half_rate, inv_f);
                ...
                if (new_synth.volume > 0.0f)
                {
                    if (new_synth.custom)
                    {
                        ...
                        new_synth.phi = last_synth.phi;
                        float const freq_base = key_to_freq(24.f);
                        float freq_factor = new_synth.freq / freq_base;
                        float main_sfx_volume = new_synth.volume;
                        update_sfx_state(channel_state.custom_sfx, new_synth, freq_factor, 0.0f, false, true, half_rate, inv_f);
                ...
                || new_synth.instrument != last_synth.instrument)
            {
                ...
                new_synth.phi = fmodf(new_synth.phi, 1.0f);
            }
            ...
            if (channel_state.fade > 0.0f)
            {
                channel_state.fade_synth.phi = channel_state.fade_synth.phi + channel_state.fade_synth.freq * inv_f;
                ...
                channel_state.fade -= 130.0f * inv_f;
            }
            ...
            if (chan_reverb1_value > 0.0f) value += chan_reverb1_value * channel_state.reverb_2[channel_state.reverb_index % 16] * 0.5f;
            if (chan_reverb2_value > 0.0f) value += chan_reverb2_value * channel_state.reverb_4[channel_state.reverb_index % 32] * 0.5f;

            channel_state.reverb_2[channel_state.reverb_index % 16] = value;
            channel_state.reverb_4[channel_state.reverb_index % 32] = value;
...
float vm::get_synth_sample(state::synth_param &params)
{
    ...
    {
        float factor = 1.0f;
        if (params.instrument == synth::INST_TRIANGLE) factor = (detune == 1) ? 3.0f / 4.0f : 3.0f / 2.0f;
        else if (params.instrument == synth::INST_ORGAN) factor = (detune == 1) ? 200.0f / 199.0f : 800.0f / 199.0f;
        else if (params.instrument == synth::INST_PHASER) factor = (detune == 1) ? 49.0f / 50.0f : 400.0f / 199.0f;
        else factor = (detune == 1) ? 200.0f / 199.0f : 400.0f / 199.0f;

        state::synth_param second_wave = params;
        second_wave.phi *= factor;
        if (detune == 2 && params.instrument == synth::INST_ORGAN) second_wave.instrument = synth::INST_TRIANGLE;
...
void vm::set_music_pattern(int pattern)
{
    ...
    m_state.music.length = (float)duration;
...
void vm::api_sfx(int16_t sfx, opt<int16_t> in_chan, int16_t offset, int16_t length)
{
    ...
    if (chan == -1)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            sfx_t const& sfx_data = m_ram.sfx[index];
    ...
    launch_sfx(sfx, chan, (float)offset, (float)length, false);
}
